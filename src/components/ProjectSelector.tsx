...
export function ProjectSelector() {
    ...
    const handleInit = () => {
        if (projectPath) initializeProject(projectPath.split('/').pop() || 'New Project');
    };
                ...
                {error && (
                    <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-lg">
                        ...
                        {error.includes('Project not initialized') && (
                            <button onClick={handleInit} type="button" className="mt-2 text-xs bg-red-500/20 hover:bg-red-500/30 text-red-300 px-2 py-1 rounded">Initialize Project</button>
                        )}
                    </div>
                )}
            </form>
